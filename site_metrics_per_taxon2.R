...
.SiteRarefiedRichness<-function (diversity, site.abundance) 
{
    ...
    study.rsr.suitable <- tapply(diversity$Measurement[diversity$Diversity_metric_is_suitable_for_Chao], 
        diversity$SS[diversity$Diversity_metric_is_suitable_for_Chao], 
        function(m) all(floor(m) == m))
    study.rsr.suitable[is.na(study.rsr.suitable)] <- FALSE
    site.rsr.suitable <- study.rsr.suitable[tapply(diversity$SS, 
    ...
    if (any(diversity$Is_RSR_suitable)) {
        siteTotalAbund <- aggregate(Measurement ~ SSS, data = droplevels(diversity[diversity$Is_RSR_suitable, 
            ]), FUN = sum)
        siteTotalAbund$SS <- diversity$SS[match(siteTotalAbund$SSS, 
            diversity$SSS)]
        studyMinSiteAbund <- aggregate(Measurement ~ SS, data = siteTotalAbund, 
            FUN = function(x) min(x[x > 0]))
        diversity$studyMinSiteAbund <- studyMinSiteAbund$Measurement[match(diversity$SS, 
            studyMinSiteAbund$SS)]
        ...
        values <- split(diversity$Measurement[diversity$Is_RSR_suitable], 
            droplevels(diversity$SSS[diversity$Is_RSR_suitable]))
        rsr <- matrix(nrow = length(which(site.rsr.suitable)), 
            ncol = 1000)
        for (i in 1:1000) {
...
.AppendSiteCWMTrait <- function(site.metrics, diversity, traits) {
  ...
  if(length(traits)>0) {
    ...
    weights <- split(diversity$Measurement, diversity$SSS) # get a list of abundance/occurrence measurements, where each dataframe is a site
    for(trait in traits) {
