...
# test as linear
# convergence warnings for these models
data <- r.sp.model.IUCN$data
m5i <- glmer(Richness_rarefied ~ 1 + poly(log_slope,1) + poly(ag_suit,1) + poly(log_elevation,1)
	+ (IUCN_CAT|SS) + (1|SSB)+ (1|SSBS), 
	family = "poisson", data = data,
	control= glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=100000)))
m6i <- glmer(Richness_rarefied ~ IUCN_CAT + poly(log_slope,1) + poly(ag_suit,1) + poly(log_elevation,1)
	+ (IUCN_CAT|SS) + (1|SSB)+ (1|SSBS), 
	control= glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=100000)),
	family = "poisson", data = data)
anova(m5i, m6i)
...
se.fixef(m4i)
se.fixef(m6i)
se.fixef(r.sp.model.IUCN$model)
#similar - since models that converge give similar estimates to modelselect output, can use model select 
...
text(1, 80, paste("n =", length(data$SSBS[which(data$IUCN_CAT == "0")]), sep = " "))
text(2, 80, paste("n =", length(data$SSBS[which(data$IUCN_CAT == "4.5")]), sep = " "))
text(3, 80, paste("n =", length(data$SSBS[which(data$IUCN_CAT == "7")]), sep = " "))
text(4, 80, paste("n =", length(data$SSBS[which(data$IUCN_CAT == "1.5")]), sep = " "))
...
IUCN.plot <- data.frame(label = labels[2:4], est = points[2:4], 
		...
		n.site = c(length(data$SSBS[which(data$IUCN_CAT == "4.5")]), 
			length(data$SSBS[which(data$IUCN_CAT == "7")]),
			length(data$SSBS[which(data$IUCN_CAT == "1.5")])))
...
abline(h= 100, lty = 2)
text(1:nrow(r.sp.plot),72, r.sp.plot$n.site, srt = 90)
...
par(mar = c(9,6,4,2))
plot(1,1, 
	ylim = c(80,145), xlim = c(0.5,nrow(r.sp.plot2)+0.1),
	bty = "l", 
	axes = F,
	ylab = "Rarefied richness difference (%)",
	cex.lab = 1.5,
...
arrows(1:nrow(r.sp.plot2),r.sp.plot2$upper,
	col = c(1,1,rep(rgb(0.5, 0.5, 0.5), 3)),
	lwd = 2,
	1:nrow(r.sp.plot2),r.sp.plot2$lower, code = 3, length = 0, angle = 90)
points(r.sp.plot2$est ~ c(1:nrow(r.sp.plot2)),
	pch = c(21, rep(16,4), rep(15,2),rep(17,3)), 
	lwd = 2,
	col = c(1,1,rep(rgb(0.5, 0.5, 0.5), 3)),
	bg = "white", 
	cex = 1.5)
text(1:nrow(r.sp.plot2),82, r.sp.plot2$n.site, srt = 90, cex = 1)
axis(1, c(1:nrow(r.sp.plot2)), r.sp.plot2$label, cex.axis = 1.1, las = 2, tick = 0)
