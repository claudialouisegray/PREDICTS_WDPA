...
PA_11_14_ord <- PA_11_14
PA_11_14_ord$IUCN_CAT <- factor(PA_11_14_ord$IUCN_CAT, ordered = T, 
	levels = c("0", "4.5", "7", "1.5"))
PA_11_14_ord$ag_suit <- factor(PA_11_14$ag_suit, ordered = T)

multiple.taxa.PA_11_14_ord <- multiple.taxa.PA_11_14
multiple.taxa.PA_11_14_ord$IUCN_CAT <- factor(multiple.taxa.PA_11_14_ord$IUCN_CAT, ordered = T, 
	levels = c("0", "4.5", "7", "1.5"))
...
# is actually a proportion of the outside PA value (as it is times by 100 to get a percentage)
...
# Species_richness~IUCN_CAT+poly(ag_suit,1)+poly(log_elevation,2)+(IUCN_CAT|SS)+(1|SSB)+(1|SSBS)
# but, doesnt converge with ag suit and elevation cubic and slope dropped
...
# m3ai doesnt converge - use ordinal stats but plot/coefficients from m4ai on non-ordinal
...
PA_11_14_amp_mam_bir$y <- cbind(PA_11_14_amp_mam_bir$abundance_VU_EN_CR, PA_11_14_amp_mam_bir$abundance_LC_NT)
...
RLS.model<- model_select(all.data  = PA_11_14_amp_mam_bir, 
			     responseVar = "y", 
			     fitFamily = "binomial",
			     alpha = 0.05,
                       fixedFactors= fF,
...
data <- PA_11_14_amp_mam_bir[,c("ag_suit", "log_elevation", "log_slope", "Within_PA", "SS", "SSB", "SSBS", "y")]
data <- na.omit(data)
...
data <- PA_11_14_amp_mam_bir[,c("Within_PA", "SSS", "y")]
data <- na.omit(data)
...
m1ti <- glmer(y ~ Within_PA + log_slope + log_elevation + ag_suit
	+ (Within_PA|SS)+ (1|SSB) + (1|SSBS), 
	family = "binomial", data = PA_11_14_amp_mam_bir)
