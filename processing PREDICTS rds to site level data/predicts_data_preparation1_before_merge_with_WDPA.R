...
source("R:/ecocon_d/clg32/GitHub/PREDICTS_WDPA/site_metrics_per_taxon2.R")


### load diversity file with PREDICTS data


setwd("R:/ecocon_d/clg32/PREDICTS/data from Lawrence")
...
### check range estimates 

species.ranges <- gbif$Values
hist(species.ranges$Value)

# 1 degree = 60minutes = 360 seconds
# if 30secs = 1km2, 1 degree = 12km2
...
#check 100 randomisations enough
#d.metrics.test <- SiteMetrics_taxa_split(diversity, extra.cols = c("Realm", "Zone", "taxon_of_interest", "Country"),  traits = traits)
#hist(round(d.metrics$Richness_rarefied) - round(d.metrics.test$Richness_rarefied))
...
writeOGR(obj = spdf, dsn = "C:/GIS/PA_predicts_mapping", "all_sites_taxa_split_coarse_11_2014", driver = "ESRI Shapefile")
